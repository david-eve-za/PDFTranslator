...
class AudioGenerator:
    def __init__(self, voice="Paulina", output_dir=Path(tempfile.mkdtemp()), final_output="final_audio.m4a"):
        """
    ...
    def text_to_audio(self, text, output_file):
        ...
        """
        try:
            subprocess.run(["say", "-v", self.voice, "-o", output_file, text], check=True)
    ...
    def merge_audio_files(self, audio_files):
        ...
        try:
            ...
            subprocess.run(
                ["ffmpeg", "-f", "concat", "-safe", "0", "-i", str(file_list), "-c", "copy", self.final_output],
                check=True
            )
    ...
    def process_texts(self, texts):
        ...
        try:
            ...
            for i, text in tqdm(enumerate(texts, start=1), desc="Processing Texts", unit="Text"):
                if "text" in text:
                    audio_file = self.output_dir / f"chunk_{i:03d}.m4a"
