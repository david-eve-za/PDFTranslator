...
class GeminiAI(LLMService):
    ...
    def __init__(self):
        ...
        if not self.config.gemini_model_names:
            raise GeminiAIInitializationError("Gemini model names list cannot be empty in config.")
    ...
    def call_model(self, prompt: str) -> str | None:
        ...
        for attempt in range(max_retries):
            ...
            self._logger.debug(f"Attempting to call model '{current_model_name}' (Attempt {attempt + 1}/{max_retries}).")
            try:
                response = self._llm_client.invoke(prompt)
                if response.content:
                    self._logger.info(f"Call to '{current_model_name}' successful. Usage: {response.usage_metadata}")
                ...
                self._logger.warning(f"Model '{current_model_name}' is exhausted. Rotating. Error: {e}")
                ...
                if self._current_model_index == initial_model_index:
                    ...
                    raise GeminiAPICallError("All available models are exhausted.") from first_exhaustion_error
                self._llm_client = self._create_llm_client_for_current_model()
            except (google_exceptions.BadRequest, google_exceptions.PermissionDenied, google_exceptions.NotFound) as e:
                self._logger.error(f"Non-retryable API error with '{current_model_name}': {e}", exc_info=True)
                raise GeminiAPICallError(f"Non-retryable API error: {e}") from e
            except Exception as e:
                self._logger.error(f"An unexpected error occurred with '{current_model_name}': {e}", exc_info=True)
                raise GeminiAIError(f"An unexpected error occurred: {e}") from e
        
        self._logger.error(f"Failed to get a valid response from model '{self.get_current_model_name()}' after {max_retries} attempts.")
        raise GeminiAPICallError("Failed to get a valid response after multiple retries.")
    ...
    def _create_llm_client_for_current_model(self) -> Runnable:
        ...
        rpm = self.config.gemini_model_rate_limits.get(model_name, self.config.gemini_default_fallback_rpm)
        requests_per_second = rpm / 60.0
        self._logger.info(f"Creating LLM client for '{model_name}' with rate limit of {rpm} RPM.")
        ...
        try:
            ...
            self._logger.error(f"Failed to initialize client for '{model_name}': {e}", exc_info=True)
            raise GeminiAIInitializationError(f"Failed to initialize client for '{model_name}': {e}") from e
    ...
    def _initialize_tokenizer(self) -> spm.SentencePieceProcessor:
        ...
        try:
            ...
            self._logger.error(f"Failed to load tokenizer model from '{tokenizer_path}': {e}", exc_info=True)
            raise GeminiAIInitializationError(f"Failed to load tokenizer model: {e}") from e
    ...
    for model in ai.models.list():
        ...
        if hasattr(model, 'supported_actions') and 'generateContent' in model.supported_actions:
            if "2.5" in model.name:
                print(f"- {model.name}")  # Nombre del recurso (ej: 'models/gemini-2.0-flash')
                print(f"  Display Name: {model.display_name}")  # Nombre para mostrar (ej: 'Gemini 2.0 Flash')